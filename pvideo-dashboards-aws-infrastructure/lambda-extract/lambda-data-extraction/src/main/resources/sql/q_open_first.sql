select client_name, procedure_name, scope, REGISTER_LETTER_NUMBER_TO as version,
 ...
 from 
	(
		select client_name, procedure_name, scope, sh.REGISTER_LETTER_NUMBER_TO,
		...
		from SHIPMENT sh 
			 join (select shipment_id, doxee_platform_id, load_date, create_date from EMAIL_OPENING_EVENT where load_date between ? and ?
