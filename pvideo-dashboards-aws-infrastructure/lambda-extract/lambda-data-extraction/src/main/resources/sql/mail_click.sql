...
from JOB jb join LOT lot on jb.job_id=lot.job_id and jb.doxee_platform_id=lot.doxee_platform_id
     ...
     join (select shipment_id, doxee_platform_id, load_date from PURL_DOWNLOAD where load_date > ?) pe
     on sh.shipment_id=pe.shipment_id and sh.doxee_platform_id=pe.doxee_platform_id
     left join
     (select shipment_id, doxee_platform_id, load_date from PURL_DOWNLOAD where load_date <= ?) pe2
