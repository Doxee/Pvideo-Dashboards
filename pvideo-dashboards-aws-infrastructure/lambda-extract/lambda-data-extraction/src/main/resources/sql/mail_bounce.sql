select jb.client_name, jb.procedure_name, jb.environment_name, sh.REGISTER_LETTER_NUMBER_TO as version,
...
from JOB jb join LOT lot on jb.job_id=lot.job_id and jb.doxee_platform_id=lot.doxee_platform_id
     ...
     join (select shipment_id, doxee_platform_id, load_date from EMAIL_DELIVERY_INTERMEDIATE where load_date > ?
                                                                                                   and event_description not in ('Delivery notification')) pe
     ...
     (select shipment_id, doxee_platform_id, load_date from EMAIL_DELIVERY_INTERMEDIATE where load_date <= ?
                                                                                              and event_description not in ('Delivery notification'))  pe2
...
group by jb.client_name, jb.procedure_name, jb.environment_name, sh.REGISTER_LETTER_NUMBER_TO
