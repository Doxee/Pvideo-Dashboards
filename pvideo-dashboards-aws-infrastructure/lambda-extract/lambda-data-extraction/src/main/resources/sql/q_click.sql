...
from SHIPMENT as sh 
     join (select shipment_id, doxee_platform_id, load_date from PURL_DOWNLOAD where load_date between ? and ?
            and client_name = ? and procedure_name = ? ) pe
          on sh.shipment_id=pe.shipment_id and sh.doxee_platform_id=pe.doxee_platform_id
     left join
           (select shipment_id, doxee_platform_id, load_date from PURL_DOWNLOAD where load_date < ?
